...
//
// Ported to the `luaur-rt` API (the mlua-style, pure-Rust ergonomic surface over
// `luaur`/Luau) as a behavioral-compatibility proof for the `serde` feature.
// Where a test passes unchanged (import swap only) against `luaur-rt`, it
// demonstrates that `luaur-rt`'s serde integration matches mlua's observable
// behavior. See `tests/ATTRIBUTION.md` for the full MIT license text.
//
// DEVIATIONS from mlua's `tests/serde.rs`:
//
// * Luau numbers are `f64`; luaur-rt reconstructs the Integer/Number split from
...
//   value. luaur-rt's `Value` has no `LightUserData` variant, so `lua.null()`
...
//   not-yet-implemented feature in luaur-rt. Tests that require it
...
use bstr::BString;
use luaur_rt::{Error, Lua, LuaSerdeExt, Result as LuaResult, Value};
...
fn test_serialize() -> Result<(), Box<dyn StdError>> {
    ...
    // implemented in luaur-rt (see the file header). Everything else is verbatim.
    let lua = Lua::new();
...
fn test_from_value_with_options() -> Result<(), Box<dyn StdError>> {
    use luaur_rt::DeserializeOptions;
...
fn test_from_value_sorted() -> Result<(), Box<dyn StdError>> {
    use luaur_rt::{DeserializeOptions, ExternalResult};
